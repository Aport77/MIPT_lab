...
class SimplifiedBaggingRegressor:
    ...
    def _get_oob_predictions_from_every_model(self):
        ...
        for i, model in enumerate(self.models_list):
            for j, data_point in enumerate(self.data):
                if j not in self.indices_list[i]:
                    list_of_predictions_lists[j].append(model.predict([data_point])[0])       
    ...
    def OOB_score(self):
        ...
        predictions = np.array([p for p in self.oob_predictions if p is not None])
        targets = np.array([t for t, p in zip(self.target, self.oob_predictions) if p is not None])
        return np.mean((predictions - targets) ** 2)# Your Code Here
